...
weekend_concierge.py
  ...
  │             (clean_source_url), which promotes the search link. Every DATED candidate also
  │             carries a `calendar_url` (build_calendar_url) — the email's one call to action.
...
| `weekend_concierge.py` | The pipeline (HARVEST→FIND×2→SKEPTIC→language gate→routine-maintenance drop→expired-notice drop→floors→anti-repeat→CONCIERGE→email). `role_for`/`score_field`/`score_of`/`floor_for`/`civic_key`/`evergreen_key(name, audience)` own the routing table above; `filter_seen()` applies the per-category cooldowns; `is_recurring_maintenance()` and `civic_notice_expired()` are the two structural civic drops; `clean_source_url()` strips bare-domain citations; `build_links()` builds each candidate's `(source_url, maps_url, search_url)` before the concierge call, and `build_calendar_url()` builds the Add-to-calendar link beside them (`parse_when_times()` pulls the start/end clock time out of `when_text`, the only field that carries one). `save_json()`/`load_json()` are this project's UTF-8-safe replacements for the `common` pair (see the UTF-8-STATE-IO invariant); `load_json` falls back to the platform encoding on a `UnicodeDecodeError` so a legacy cp1252 state file is read rather than silently discarded, and the next save rewrites it as UTF-8. `degraded_summary(stage_results)` returns the `(subject_prefix, banner_html, banner_text, log_line)` for a run where a stage failed outright — `stage_results` is a list of `(stage_name, LLMResult)` accumulated at each of the four call sites. Tests: `test_concierge.py` (offline, stubs `llm_chain.call_llm`/`scrapers.harvest`/`weather.week_weather`, runs `main()` twice to verify state files + suppression + per-audience evergreen rotation, plus unit tests for the `civic\|` prefix, schema↔`role_for` coverage, weather field names, audience isolation, the score sort key, bare-domain stripping, the maintenance and expiry drops, that an unverified item is sent *and* flagged, the SKEPTIC infra-vs-empty split, UTF-8 state IO, `parse_when_times()`, `build_calendar_url()`'s timed/all-day/undated cases, and that a real `calendar_url` actually reaches the concierge prompt). |
...
- **CALENDAR-IS-THE-CTA: every dated item gets an Add-to-calendar button, and it is the one
  link exempt from "not on every line".** The email had no call to action at all: the owner
  read about a festival three weeks out and then had to retype its title, date, time and venue
  into Google Calendar by hand, which in practice means it was forgotten. `build_calendar_url()`
  emits a `calendar.google.com/calendar/render?action=TEMPLATE…` link per candidate and
  `CONCIERGE_PROMPT` carries the exact button snippet to paste, so the model is never composing
  a URL. Three rules hold it together:
  - **Undated items get `""`, and the prompt must never fabricate one.** Evergreens and most
    `civic_opportunity` items have no date; there is nothing to put on a calendar, and a button
    that adds "Children's Railway" to today would be worse than no button.
  - **Timed only when `when_text` yields a start time AND the item is single-day**
    (`parse_when_times()`); everything else is all-day. `when_text` is free prose and the only
    field carrying a clock time — neither FIND schema has one — so both FIND prompts now ask for
    a 24-hour start time in it. Two separate showings ("10:30 & 11:45") deliberately do **not**
    read as a range: only an explicit dash or "to" does. A date range ("26-27") has no colon and
    so can never be misread as a time.
  - **All-day end dates are EXCLUSIVE** in Google's URL format, so a 26–27 event must be sent as
    `dates=20260926/20260928`. Off-by-one here silently drops the last day of every festival.
  Local wall-clock times are passed with `ctz=Europe/Sofia` rather than converted to UTC — no DST
  arithmetic happens in this repo, deliberately. `end_date_iso` was added to
  `STAGE1_FAMILY_SCHEMA` for this (the adult schema already had it); it is optional, and
  `civic_notice_expired()` is gated on `category == "civic_notice"`, so a family item carrying
  one cannot be dropped by the expiry check.
